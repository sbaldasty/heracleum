...
OUTPUT_FILE = './out/poisoneffect/honest.csv'
N_HONEST_CLIENTS = 0
N_ROUNDS = 30
N_CORRUPT_CLIENTS_START = 5
...
if __name__ == '__main__':
    ...
    for (attack_name, attack_obj), (defense_name, defense_obj), n_corrupt in product(attacks, defenses, corrupt_clients_range):
        ...
        server, model = make_cifar_server(attack_obj, defense_obj, accusation_counter, corrupt_client_ids, n_clients=n_corrupt)

        start_simulation(
            client_fn=client_fn,
            num_clients=N_HONEST_CLIENTS + n_corrupt,
            server=server,
